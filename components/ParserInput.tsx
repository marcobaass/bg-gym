...
export default function ParserInput() {
  ...
  useEffect(() => {
    async function loadFolders() {
      ...
      const exists = lastId
        ? initialFolders.find((folder) => folder.category.id === lastId)
        : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        positionData.bestMoves.length === 1
      ) {
      ...
      if (
        positionData.analysisType === "Cube" ||
        positionData.analysisType === "Move"
      ) {
        // Phase 2 & 3 saving
        ...
        if (categoryId === SENTINEL) {
          ...
          if (
            isCategoryNameTaken(
              folders.map((folder) => folder.category),
              newCategoryNameTrimmed,
            )
          ) {
          ...
          if (user) {
            await insertCategory(supabase, user, {
              id: newId,
    ...
    <div className="flex justify-center p-4">
      <div className="w-full max-w-sm bg-white p-6 rounded-xl shadow-md space-y-4 border border-indigo-100">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <select
            id="categorySelect"
            className="w-full border border-gray-300 p-3 rounded-lg text-gray-900 placeholder-gray-400 focus:ring-2 focus:outline-none transition duration-150 ease-in-out mb-4 focus:ring-indigo-500 focus:border-indigo-500"
            value={categoryId}
