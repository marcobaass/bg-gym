...
export function compareWithBestMoves(
  ...
  const foundMatch = normalizedBestMoves.find((move) =>
    areMovesSame(normalizedUserMoves, move.move),
  );
...
function normalizeBestMoves(
  ...
  // 25 is the off point for black and must be converted to 24
  else {
    copyBestMoves.forEach((bestMove) => {
      bestMove.move.forEach((singleMove) => {
        if (singleMove[0] >= 0) {
        ...
        if (singleMove[1] >= 0 && singleMove[1] !== 25) {
          singleMove[1] = 24 - singleMove[1];
        } else if (singleMove[1] === 25) {
          singleMove[1] = 24;
        }
      });
    });
