import {
  ...
  CubeActions,
  CubeDecision,
  ParsedCubeDecisionOption,
  ParsedCubeDecisionSummary,
} from "@/types/board";
...
function normalizeAction(label: string): CubeDecision | null {
  ...
  if (lower === "double/take" || lower === "redouble/take")
    return "Double/Take";
  if (lower === "double/pass" || lower === "redouble/pass")
    return "Double/Pass";
...
export function buildCubeDecisionsSummary(
  ...
  if (normalizedBestLower.startsWith("too good")) {
    ...
    normalizedBestLower.includes("no double") ||
    normalizedBestLower.includes("no redouble")
  ) {
    bestDecision = "No Double";
  } else if (
    normalizedBestLower.includes("double/take") ||
    normalizedBestLower.includes("redouble/take")
  ) {
    bestDecision = "Double/Take";
  } else if (
    normalizedBestLower.includes("double/pass") ||
    normalizedBestLower.includes("redouble/pass")
  ) {
