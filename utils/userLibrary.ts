...
export function isCategoryNameTaken(userCategories: Category[], name: string) {
  ...
  if (
    userCategories.some(
      (category) => category.name.toLowerCase().trim() === inputName,
    )
  ) {
    return true;
  } else {
...
export function saveCategorySession(newSession: CategorySession): void {
  ...
  updatedCategorySessions.sort(
    (a: CategorySession, b: CategorySession) => b.finishedAt - a.finishedAt,
  );
...
export function getCategoryAverageScore(
  ...
  const sum = categorySessions.reduce(
    (acc: number, session: CategorySession) => acc + session.scorePerPosition,
    0,
  );
...
export async function loadUserLibraryFromSupabase(
  ...
  const userLibrary: UserLibrary = {
    library: categories.map((category) => {
      const categoryPositions = positions.filter(
        (position) => position.category_id === category.id,
      );
      return {
        category: category,
...
export async function loadSessionHistoryFromSupabase(
  ...
  const { data: categorySessions, error: categorySessionsError } =
    await supabase
      ...
      .select(
        "category_id, finished_at, positions_played, score_per_position, raw_total_score, id",
      );
  ...
  categorySessions.forEach((session) => {
    sessionsByCategory[session.category_id] = [
      ...(sessionsByCategory[session.category_id] ?? []),
      {
        id: session.id,
...
export async function importLocalStorageToSupabase(
  ...
  const knownCategoriesCloud = cloudLibrary.library.map(
...
export async function insertCategoryToSupabase(
  supabase: SupabaseClient,
  userId: string,
  category: Category,
): Promise<void> {
  const { error } = await supabase.from("categories").insert({
    ...
    name: category.name,
    visibility:
      userId === process.env.NEXT_PUBLIC_CURATOR_USER_ID ? "system" : "private",
  });
...
export async function insertPositionToSupabase(
  supabase: SupabaseClient,
  userId: string,
  categoryId: string,
  position: Position,
...
export async function saveCategorySessionToSupabase(
  ...
  const { error } = await supabase.from("category_sessions").insert({
    id: session.id,
    category_id: session.categoryId,
    user_id: userId,
    finished_at: session.finishedAt,
